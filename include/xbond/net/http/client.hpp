...
public:
    ...
    void execute(const core::address& addr, 
        ...
        if (!stream_.socket().is_open()) {
            ...
            resolver_.async_resolve(address_.host(), address_.service(),
                [&ch, &ep] (const boost::system::error_code& error, tcp::resolver::results_type rs) {
            ...
            boost::beast::get_lowest_layer(stream_).async_connect(ep, 
                [&ch] (const boost::system::error_code& error, tcp::resolver::endpoint_type ep) {
                ch.resume(error);
            });
            ch.yield(error);
            if (error) {
            ...
            tcp::socket::keep_alive keep_alive(true);
            boost::beast::get_lowest_layer(stream_).socket().set_option(keep_alive);
        }
        buffer_.clear();
        // 发送请求
        req.prepare_payload();
        http::async_write(stream_, req, ch[error]);
        if (error) {
        ...
        // 接收响应
        http::async_read(stream_, buffer_, rsp, ch[error]);
        if (error) {
